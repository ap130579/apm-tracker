...
def norm_key(company, title):
    return (company.lower().strip(), re.sub(r"\s+", " ", title.lower().strip()))
...
def cmd_scan(args):
    ...
    for name, postings in results.items():
        for p in postings:
            ...
            seen_keys.add(key)
            if key not in known:
                new_postings.append({"company": name, "source": "ats-api", **p})
    ...
    print(json.dumps(
        {
            ...
            "companies_checked_via_api": len(results),
            "fetch_errors": errors,
            "new_postings": new_postings,
...
def cmd_finalize(args):
    ...
    if news:
        for p in sorted(news, key=lambda x: x["company"]):
            date_bit = f" — posted {p['posting_date']}" if p.get("posting_date") else ""
            lines.append(f"- **{p['company']}** — {p['role_title']}{date_bit} — {p.get('url', '')}")
    else:
    ...
    n_companies = len(pending["checked_companies"]) + len(pending.get("manual_check", []))
    lines += ["## Summary", f"{len(news)} new posting(s) found across {n_companies} companies checked."]
    digest = "\n".join(lines) + "\n"
